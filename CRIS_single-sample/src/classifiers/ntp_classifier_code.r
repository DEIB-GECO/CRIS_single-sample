...
#        eval(parse(text=paste("current\.temp <- temp\.",o,sep="")))  ### for < R-2.4.0
        orig.dist.to.all.temp[o]<-sum(current.temp*current.sample, na.rm = T)/
                  (sqrt(sum(current.temp^2))*sqrt(sum(current.sample^2,na.rm = T)))
      }
      #print(1-orig.dist.to.all.temp)
      out <- cbind(out, orig.dist.to.all.temp)
    }
...
#        eval(parse(text=paste("current\.temp <- temp\.",o,sep="")))  ### for < R-2.4.0
    ...
    combined.stats.rank.all <- vector() 
    #print(orig.dist.to.all.temp)
 
    for(o in 1:length(orig.dist.to.all.temp)){
       combined.stats.rank.all[o] <- rank(c(1-orig.dist.to.all.temp[o],perm.dist.vector))
    }
    #print(dim(combined.stats.rank.all))
#    print(dist.to.template[i])
    nominal.p[i]<-combined.stats.rank[1]/length(combined.stats.rank)
#    print(nominal.p[i])
    nominal.p.all <- combined.stats.rank.all/length(combined.stats.rank)
    nominal.p.all.out <- cbind(nominal.p.all.out, nominal.p.all)
#    print(nominal.p.all)
#    print(combined.stats.rank[1])
  ...
  print(dim(nominal.p.all.out))
  #return(list(nominal.p.all.out, nominal.p))
  BH.FDR.all.out <- vector()
  for(o in 1:dim(nominal.p.all.out)[2]){
    
     BH.FDR.all.tmp <- vector()
     for(i in 1:dim(nominal.p.all.out)[1]){
        BH.FDR.all.tmp2 <- nominal.p.all.out[i,o]*num.samples/rank(nominal.p)[o]
        BH.FDR.all.tmp <- c(BH.FDR.all.tmp, BH.FDR.all.tmp2)
#        print(BH.FDR.all.tmp)
     }
#     print(BH.FDR.all.tmp)
     BH.FDR.all.out <- cbind(BH.FDR.all.out, BH.FDR.all.tmp)
  }
  print("BH")
  print(dim(BH.FDR.all.out) )
  ...
  pred.summary <- cbind(sample.names,predict.label,dist.to.template,t(1-out),dist.to.cls1.rank,
              nominal.p, t(nominal.p.all.out),BH.FDR, t(BH.FDR.all.out),Bonferroni.p)
  pred.summary[which(pred.summary[,2] == "1"),2] <- "CRIS-A"
  pred.summary[which(pred.summary[,2] == "2"),2] <- "CRIS-B"
  pred.summary[which(pred.summary[,2] == "3"),2] <- "CRIS-C"
  pred.summary[which(pred.summary[,2] == "4"),2] <- "CRIS-D"
  pred.summary[which(pred.summary[,2] == "5"),2] <- "CRIS-E"
  colnames(pred.summary)[c(4:8)] <- c("distCRIS-A", "distCRIS-B", "distCRIS-C", "distCRIS-D", "distCRIS-E")
  colnames(pred.summary)[c(11:15)] <- c("nominal.p.CRIS-A", "nominal.p.CRIS-B", "nominal.p.CRIS-C", "nominal.p.CRIS-D", "nominal.p.CRIS-E")
  colnames(pred.summary)[c(17:21)] <- c("BH.FDR.CRIS-A", "BH.FDR.CRIS-B", "BH.FDR.CRIS-C", "BH.FDR.CRIS-D", "BH.FDR.CRIS-E")
  #return(pred.summary)
