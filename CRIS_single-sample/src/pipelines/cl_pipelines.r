...
#   attr(pred, 'classes') <- ml_pred_binary[,levels(F_CRIS_CLASSES)]
#   
#   print_info('Metrics...')
...
# 
# 
# sl_pipeline_test <- function(sldata, method, seed, model, cl_thresholds, png_path, mldata = NULL, max_cls = NULL, min_cls = NULL){
#   
#   # Declare classifier
...
#     pred <- ohenery::inv_smax(as.matrix(sl_pred[,levels(F_CRIS_CLASSES)]))
#       
...
#     print_info('Metrics...')
#     mlm <- MLMetrics$new()
#     dfres <- sl_pred_binary[,levels(F_CRIS_CLASSES)]
#     mlres <- as.mlresult(sl_pred[,levels(F_CRIS_CLASSES)])
...
#     pred <- ohenery::inv_smax(as.matrix(pred[,levels(F_CRIS_CLASSES)]))
#     
#     for (c in levels(F_CRIS_CLASSES)){
